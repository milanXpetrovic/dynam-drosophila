...
stan_file        <- file.path(SRC_DIR, "dynam_map_reduce_event.stan")
treatments_list  <- c("CS_10D", "Cs_5DIZ", "CsCh")

if (!dir.exists(edgelist_path))
  stop("No data/edgelists under PROJECT -> fix the PROJECT path at the top.\n  looked in: ", edgelist_path)
...
parallel_results <- foreach(
  ...
  cov_distance             <- read_and_clean("distance_traveled_between_interactions.csv", group_cov_path)
  cov_n_soc                <- read_and_clean("number_of_flies_in_soc_space.csv", group_cov_path)
  cov_uniq_int             <- read_and_clean("unique_partners_met_interaction_space.csv", group_cov_path)
  cov_uniq_soc             <- read_and_clean("unique_partners_met_social_space.csv", group_cov_path)
  ...
  list(group_name = group_name, treatment_name = treatment_name,
       group_index = i, data_object = data_choice)
}
stopCluster(cl)
registerDoSEQ()
...
for (result in parallel_results) {
  if (inherits(result, "list") && !is.null(result$data_object)) {
    ...
    message("Group failed: ", paste(utils::capture.output(print(result)), collapse = " "))
  }
}
...
stopifnot(
  ...
  any(grepl("alter_fliesOf", .cn, fixed = TRUE))
)
...
int_formula   <- as.formula(paste0(
  ...
  paste(termsInteract, collapse = " + "), ") + 0"
))

old_X_choice <- data_stan_re$data_stan$X_choice
var_distance <- grep("distance", colnames(old_X_choice), value = TRUE)
...
data_stan_re$scale <- list(center = attr(new_X_scale, "scaled:center"),
                           scale  = attr(new_X_scale, "scaled:scale"))

Z_scale <- scale(data_stan_re$data_stan$Z_choice)
data_stan_re$scale_Z <- list(center = attr(Z_scale, "scaled:center"),
...
message("STEP 2 done: design matrix ", nrow(new_X_choice), " x ", ncol(new_X_choice))
