...
parallel_results <- foreach(
...
) %dopar%
    {
        ...
        cov_distance_traveled_between_interactions <- read_and_clean(
            'distance_traveled_between_interactions.csv',
            group_cov_path
        )
        cov_number_of_flies_in_soc_space <- read_and_clean(
            'number_of_flies_in_soc_space.csv',
            group_cov_path
        )
        cov_unique_partners_met_interaction_space <- read_and_clean(
            'unique_partners_met_interaction_space.csv',
            group_cov_path
        )
        cov_unique_partners_met_social_space <- read_and_clean(
        ...
        nodesAttr <- link_events(
            nodesAttr,
            cov_distance_traveled_between_interactions,
            'distance_traveled_between_interactions'
        )
        nodesAttr <- link_events(
            nodesAttr,
            cov_number_of_flies_in_soc_space,
            'number_of_flies_in_soc_space'
        )
        nodesAttr <- link_events(
            nodesAttr,
            cov_unique_partners_met_interaction_space,
            'unique_partners_met_interaction_space'
        )
        nodesAttr <- link_events(
            nodesAttr,
            cov_unique_partners_met_social_space,
...
for (result in parallel_results) {
    if (inherits(result, "list")) {
        # Check if the result is a list (not an error)
...
for (group_name in names(all_groups_data_list)) {
    fixed_effects_matrix <- all_groups_data_list[[
        group_name
    ]]$data_stan$X_rate[, -1, drop = FALSE]
...
relevant_vars_idx <- (rowSums(final_count_matrix) > 0) |
    (colSums(final_count_matrix) > 0)
matrix_to_plot <- final_count_matrix[relevant_vars_idx, relevant_vars_idx]
png(
    "high_correlation_plot_rate09.png",
    width = 14,
    height = 14,
    units = "in",
    res = 300
...
whichDistance <- grep(
    "ego_nodesAttrOfdistance_traveled_between_interactions",
    names(dataCox)
)
...
termsInteract <- names(dataCox)[seq.int(
    2,
    which(names(dataCox) == "ego_fliesOfyoung") - 1
)]
...
remotes::install_github(
    "auzaheta/goldfish.latent@develop",
...
all_groups_data_list <- qread(
    "/home/milky/droso-pipe/6_dynam_models/all_groups_data_list_rate.qs"
)
all_groups_info <- readRDS(
    "/home/milky/droso-pipe/6_dynam_models/all_groups_info_rate.rds"
)
...
## remove effects if there
## remove experimental condition dummies
effects_remove <- c(
    "Intercept",
    "indeg_interaction_network_W_288",
    "indeg_interaction_network_W_864",
    "indeg_interaction_network_W_2592",
    "outdeg_interaction_network_W_288",
    "outdeg_interaction_network_W_864",
    "outdeg_interaction_network_W_2592",
    "node_trans_interaction_network_96",
    "node_trans_interaction_network_288",
    "node_trans_interaction_network_864",
    "node_trans_interaction_network_2592"
)
...
pos_remove <- (colnames(old_X_rate) %in% effects_remove) |
    (colnames(old_X_rate) %in% interaction_terms)
...
data_stan_re$data_stan$interaction <- c(
    ...
    rep(
        2,
        data_stan_re$data_stan$start_group[40] -
            data_stan_re$data_stan$start_group[20]
    ),
    rep(
        3,
        data_stan_re$data_stan$T_rate - data_stan_re$data_stan$start_group[40]
    )
)
...
mod_rate <- cmdstan_model(
    "/home/milky/droso-pipe/6_dynam_models/DNRE_Q0_rate_int_centered.stan",
...
random_effects_model$save_object(
    file = "tmp/full_random_effects_model_rate.rds"
)
arrayDraws <- random_effects_model$draws()
saveRDS(arrayDraws, file = "tmp/arrayDraws_rate.rds")

draws_beta <- random_effects_model_wsw$draws(
    "beta_rate",
    format = "draws_matrix"
)
...
write.csv(
    beta_intervals_original,
    file = "tmp/beta_intervals_original_rate.csv"
)

gamma_intervals <- mcmc_intervals_data(draws_gamma)
write.csv(gamma_intervals, file = "tmp/gamma_intervals_rate.csv")
